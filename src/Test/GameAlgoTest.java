...
class GameAlgoTest {
	...
	private static final ArrayList<Fruits> list = new ArrayList<Fruits>();
	 ...
	 */
	 @BeforeAll
		static void init(){
		 ...
		 list.add(new Fruits(f_iter.next()));
		 	}
		 // add robot
		 _game.addRobot(0);
	...
	void ClosestFruitTest() {
		Fruits expected = list.get(0);
		Fruits actual = gameAlgo.ClosestFruitbyShortestpath(list, r);
	...
	@Test
	void MostValueTest() {
		Fruits expected = list.get(1);
		Fruits actual = gameAlgo.mostValue(list);
